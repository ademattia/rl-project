...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Parallel orchestrator for train.py (Actor-Critic)"
    )
    ap.add_argument("--python", default="python")
    ap.add_argument("--script", default="train.py")
    ap.add_argument("--n-episodes", type=int, default=15000)
    ap.add_argument("--seeds", type=int, nargs='+', default=[15, 8, 17])
    ap.add_argument("--batch-sizes", type=int, nargs='+', default=[1, 10, 50])
    ap.add_argument("--modes", default=["baseline", "TD"])
    ...
    ap.add_argument("--status-file", default="run_status_ActorCritic.json")
...
def build_cmd(
    ...
    print("RECAP: ", n_episodes, batch_size, mode, video_every, seed, device)
...
def stream_tee(proc, log_path, prefix):
    ...
    with open(log_path, "w", buffering=1) as f:  # line-buffered file
...
def main():
    ...
    runs = [{"batch_size": b, "seed": s, "mode": m} for b in args.batch_sizes for m in args.modes for s in args.seeds]
    total_runs = len(runs)
    ...
    try:
        while pending or running:
            while pending and len(running) < args.max_procs:
                ...
                seed = job["seed"]
                mode = job["mode"]
                log_name = f"batch_{batch_size}_seed_{seed}_{mode}.log"
                ...
                pid = proc.pid
                print(
                    f"\n[launch] BATCH_SIZE={batch_size} MODE={mode} SEED={seed} DEVICE={device}"
                ...
                if args.log_mode == "tee":
                    prefix = f"[run b={batch_size} s={seed} m={mode}]"
                    tee_thread = Thread(
            ...
            if running:
                ...
                for pid in list(running.keys()):
                    ...
                    if ret == 0:
                        ...
                        print(
                            f"[done] pid={pid} batch_size={meta['batch_size']} seed={meta['seed']} mode={meta['mode']} device={meta['device']} OK"
                        )
                        ...
                        meta["returncode"] = ret
                        print(
                            f"[fail] pid={pid} batch_size={meta['batch_size']} seed={meta['seed']} mode={meta['mode']} device={meta['device']} rc={ret}"
                        )
